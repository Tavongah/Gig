#!/usr/bin/env bash
# Expand existing Let's Encrypt cert to include APP_DOMAIN (app.gigflow.ink).
# Run once on the Droplet after DNS for APP_DOMAIN points here.
set -euo pipefail
...
echo "Expanding certificate for $API_DOMAIN to include $APP_DOMAIN..."
