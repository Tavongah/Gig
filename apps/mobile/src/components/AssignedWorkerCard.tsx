  ...
  return (
    <DutsCard className="gap-4 p-5">
      ...
      <View className="flex-row gap-2">
        <Pressable onPress={onMessage} className="flex-1 rounded-2xl border border-border bg-card px-4 py-3">
          <Text className="text-center font-black text-ink">Message Worker</Text>
        </Pressable>
        {phoneNumber ? (
          <View className="flex-1">
            <AppButton label="Call Worker" onPress={() => void Linking.openURL(`tel:${phoneNumber}`)} size="md" />
          </View>
        ) : null}
      </View>
    </DutsCard>
